 ...
 *
 * The whole operation runs in one transaction with a `FOR UPDATE` lock on the
 * partner transaction row, so concurrent refunds serialize: the second one
 * re-reads the updated `refundedCents` and can never double-clawback.
 *
